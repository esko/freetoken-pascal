...
- `qwen38-long-horizon-contract.json` is the H0 contract for five semantic/agentic probe
  ...
  The contract and harness are synthetic scaffolding; the linked sensitive control proves that
  accumulated GDN drift fails even when semantic output tokens remain unchanged, but neither
  file claims model or P4 parity.
