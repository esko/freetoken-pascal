...
requires source/lossless runtime precision by default. Any representation change needs
explicit per-class qualification evidence. Existing Q8 source bytes remain visible but
block serving qualification without evidence, and are never relabeled as F32. Sub-8-bit sensitive formats,
