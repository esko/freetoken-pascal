...
The issue #11 tiny composition oracle now drives the registered
`Qwen4ExpForCausalLM` through the shipping embedding, hyperconnection, GDN,
...
The H0 placement fixture uses `freetoken.engine.placement_plan` with exact versioned categories, explicit unique GPU identities, positive-capacity/zero-free inputs, asymmetric one- and two-GPU capacities, cache-zero profiles, profile-bound synthetic post-load and post-first-large-prefill observations in order, separate driver/allocator counters, allocator consistency checks, absolute live/peak agreement, child/aggregate tolerance matching bounded by reserve, and deterministic monotonic backoff that requires both readiness checkpoints.
The placement-profile fixture additionally proves canonical sorted UTF-8 serialization, exact round-trips, digest tamper rejection, strict identity/geometry/topology validation, field-specific stale reasons, and rejection of every model, quant, runtime, driver, geometry, topology, plan or backoff mutation.
...
probe families, validates every declared turn, and requires at least 16 aligned steps of
continuation-token, router-ID, semantic-output-token, and GDN-state evidence. The
`compare_long_horizon_bundles` wrapper reuses the independent identity and A/B checks above;
the `gdn_state` numeric comparison is deliberately retained even when semantic token outputs
remain identical, so the linked perturbed GDN control cannot be hidden by fluent short or
retrieval output. This is synthetic H0 contract evidence only, not model-quality or P4
qualification.
