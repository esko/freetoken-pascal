...
5. For physical-I/O evidence, pass the explicit dedicated PLE payload file to the Linux sysfs block probe and record its terminal `major:minor/name` identity, `/sys/.../stat` field-3 source detail, and fixed 512-byte stat-sector unit. The probe treats a partition as terminal, follows only one sysfs slave for non-partition mappings, and fails readiness for overlay/tmpfs, missing or cyclic mappings, malformed counters, or multiple slaves rather than aggregating devices.
...
`physical_block_device_bytes`; a process-only counter cannot establish device
traffic. If the payload is on overlay/tmpfs or the sysfs graph has no single
terminal device, omit physical-I/O evidence and fix the storage layout before
